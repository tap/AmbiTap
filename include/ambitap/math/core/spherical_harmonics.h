...
namespace ambitap {
    ...
    ///
    /// @param order      Ambisonics order. Must be <= k_max_order.
    ...
    inline void evaluate_sh(int order, float azimuth, float elevation, float* out) {
        ...
        // k_max_order; an out-of-range order would silently corrupt the stack.
        ...
        assert(order >= 0 && order <= k_max_order);
        order = std::clamp(order, 0, k_max_order);
        ...
        // for all n <= order, 0 <= m <= n. Stored as a flat (k_max_order+1) x (k_max_order+1)
        // table; we only need m >= 0 since the m < 0 normalization is absorbed in sn3d_factor.
        const int N = order;
        float     plm[(k_max_order + 1) * (k_max_order + 1)];

        // Sectoral recurrence: P_m^m
        plm[0] = 1.0f; // P_0^0 = 1
        for (int m = 1; m <= N; ++m) {
            plm[m * (k_max_order + 1) + m] =
                static_cast<float>(2 * m - 1) * cos_el * plm[(m - 1) * (k_max_order + 1) + (m - 1)];
        }
        ...
        for (int m = 0; m <= N; ++m) {
            for (int n = m + 2; n <= N; ++n) {
                plm[n * (k_max_order + 1) + m] =
                    (static_cast<float>(2 * n - 1) * sin_el * plm[(n - 1) * (k_max_order + 1) + m]
                     - static_cast<float>(n + m - 1) * plm[(n - 2) * (k_max_order + 1) + m])
        ...
        for (int n = 0; n <= N; ++n) {
            for (int m = -n; m <= n; ++m) {
                ...
                const float p     = plm[n * (k_max_order + 1) + abs_m];
